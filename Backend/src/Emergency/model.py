...
class EmergencySession(Base):
    ...
    owner = relationship("UserModel", back_populates="emergency") # showing connection with user table

    location = relationship("LocationHistory", back_populates="session", passive_deletes=True, cascade="all, delete", order_by="LocationHistory.timestamp")


class LocationHistory(Base):
    __tablename__ = "location_history"
    id = Column(Integer, primary_key=True)
    session_id = Column(String, ForeignKey("EmergencySession.session_id", ondelete="CASCADE"))

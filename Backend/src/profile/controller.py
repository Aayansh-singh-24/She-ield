...
class Storage:
    ...
    async def save(self,file:fastapi.UploadFile) -> str:
...
class ProfileService:
    ...
    @staticmethod
    def _validate_image(file: fastapi.UploadFile):

        if not file.content_type or not file.content_type.startswith("image/"):
            raise fastapi.HTTPException(status_code=fastapi.status.HTTP_400_BAD_REQUEST,detail="Invalid image type")

    async def upload_profile(self,file:fastapi.UploadFile):
    ...
    def get_profile(self):
        ...
        if not profile:
            raise fastapi.HTTPException(status_code=fastapi.status.HTTP_404_NOT_FOUND, detail="Profile not found")


        if not os.path.exists(profile.filepath):
            raise fastapi.HTTPException(status_code=fastapi.status.HTTP_404_NOT_FOUND, detail="profile path not exist")
...
class UserService:
    ...
    def update_user_credentials(self,body: UpdateUserSchema) -> UpdateUserResponseSchema:
        ...
        if "username" in data:
            ...
            if existing_user and data['username'] == self.current_user.username:
                raise fastapi.HTTPException(status_code=fastapi.status.HTTP_400_BAD_REQUEST,detail="Username already exists")
    ...
    def update_password(self,body:UpdatePassword)-> None:
        ...
        if not verify_password(data["current_password"], self.current_user.hash_password):
            raise fastapi.HTTPException(status_code=fastapi.status.HTTP_400_BAD_REQUEST, detail="Current Password is incorrect")
               
        if verify_password(data["new_password"], self.current_user.hash_password):
            raise fastapi.HTTPException( status_code=fastapi.status.HTTP_400_BAD_REQUEST, detail="New password cannot be the same as the current password.")
               
           
        if data["new_password"] != data["confirm_password"]:
            raise fastapi.HTTPException(status_code=fastapi.status.HTTP_400_BAD_REQUEST, detail="password not matched")
